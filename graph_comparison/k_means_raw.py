...
def load_cell_features(data_path="./cell_feature_naive"):
    feature_data = np.load(f"{data_path}/cell_features.npz")
    cell_ids = feature_data['cell_ids']
    features_raw = feature_data['features']  # This will now be (num_cells, height, width)
    coords = feature_data['coords']
    
    # Flatten the spatial dimensions to create a feature vector for each cell
    num_cells = features_raw.shape[0]
    flattened_features = features_raw.reshape(num_cells, -1)
    
    with open(f"{data_path}/data_splits.json", 'r') as f:
        data_splits = json.load(f)
    
    return cell_ids, flattened_features, coords, data_splits

def create_kmeans_naive_graph(cell_features, cell_coords, n_clusters=None, k_neighbors=10, max_distance=50, 
    ...
    if cache_path and cache_id:
        cache_file = f"{cache_path}/kmeans_naive_graph_cache_{cache_id}.pkl"
    ...
    print(f"Creating K-means naive graph for {len(cell_coords)} cells...")
...
def main():
    data_path = "./cell_feature_naive"
    cell_ids, features, coords, data_splits = load_cell_features(data_path)
    
    csv_file = "/playpen/jesse/HE_IF/graph_comparison/cell_feature_naive/CRC03_new_coordinates.csv"
    ...
    cache_dir = "./graph_cache_naive"
    os.makedirs(cache_dir, exist_ok=True)
    
    # Use the K-means graph construction with caching
    G_train, train_data = create_kmeans_naive_graph(
        train_features, 
        train_coords, 
    ...
    G_val, val_data = create_kmeans_naive_graph(
        val_features, 
        val_coords, 
    ...
    G_test, test_data = create_kmeans_naive_graph(
        test_features, 
        test_coords, 
    ...
    output_dir = "naive_kmeans_gnn_results"
    epochs = 50
    patience = 15
    lr = 0.001
    weight_decay = 1e-5
...
# CUDA_VISIBLE_DEVICES=5 nohup python k_means_raw.py > kmeans_raw.log 2>&1 &