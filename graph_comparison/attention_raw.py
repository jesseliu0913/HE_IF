...
def create_attention_graph(cell_features, cell_coords, n_clusters=None, k_neighbors=10, max_distance=50, 
    ...
    for i in tqdm(range(0, num_cells, batch_size), desc="Processing batches"):
        ...
        # Find k nearest neighbors for each cell in the batch based on spatial coordinates
        nn = NearestNeighbors(n_neighbors=k_neighbors+1)  # +1 because the point itself is included
        nn.fit(cell_coords)  # Fit on all coordinates
    ...
    # Get edge weights for potential future use
    ...
    # Include edge weights in the data object
...
def main():
    ...
    output_dir = "naive_attnetion_gnn_results"
    epochs = 50
    patience = 15
    lr = 0.001
    weight_decay = 1e-5
...
# CUDA_VISIBLE_DEVICES=5 nohup python attention_raw.py > attention_raw.log 2>&1 &
# CUDA_VISIBLE_DEVICES=5 python attention_raw.py 