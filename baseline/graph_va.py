...
def construct_cell_graph(df, he_image, biomarkers, feature_extractor=None):
...
def train_gnn_pipeline(data_directory, biomarkers, batch_size=128, num_epochs=30, hidden_dim=256):
    """
    Training pipeline for biomarker prediction using GNN on a single H&E image
    with Voronoi-based cell adjacency graphs
    """
    ...
    print("Constructing Voronoi adjacency cell graph from H&E image...")
    ...
    output_dir = os.path.join(output_directory, "model_results_gnn_voronoi")
...
if __name__ == "__main__":
    ...
    train_gnn_pipeline(data_directory, biomarkers, batch_size=batch_size, num_epochs=30)